...
> Automated X.509 certificate provisioning and renewal (ACME, self-signed, Vault PKI) for every ingress endpoint in the reference architecture.
...
# Install this chart
helm install cert-manager cigip-upv/platform-cert-manager \
  --namespace platform \
  --create-namespace
```
...
helm install cert-manager . -n platform --create-namespace
```
