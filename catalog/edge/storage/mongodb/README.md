# MongoDB (Edge Buffer): `edge-mongodb`

> Document buffer with TTL indexes for automatic cleanup of raw sensor events at the edge.
...
- **ISO/IEC 42001 Annex B clauses covered**: `B.6.2.6.1`

This chart packages **MongoDB (Edge Buffer)** for the **edge** tier of the reference architecture, covering the *Storage* capability block. It ships with defaults for K3s and a Rancher-compatible `questions.yaml`, so operators can deploy the component from the Rancher UI with guided prompts.
...
# Install this chart
helm install edge-mongodb cigip-upv/edge-mongodb \
  --namespace edge \
  --create-namespace
```
...
cd MLOps-ISO42001-K3s-Catalog/catalog/edge/storage/mongodb/manifests
helm dependency build .
helm install edge-mongodb . -n edge --create-namespace
```
...
```bash
kubectl get pods,svc,deploy,sts -A -l mlops-iso42001.cigip-upv.es/B.6.2.6.1
```
