...
This chart packages **MongoDB (Edge Buffer)** for the **edge** tier of the reference architecture, covering the *Storage* capability block. It ships with production-ready defaults for K3s and a Rancher-compatible `questions.yaml`, so operators can deploy the component from the Rancher UI with guided prompts for every configuration variable.
...
# Install this chart
helm install mongodb cigip-upv/edge-mongodb \
  --namespace edge \
  --create-namespace
```
...
helm install mongodb . -n edge --create-namespace
```
