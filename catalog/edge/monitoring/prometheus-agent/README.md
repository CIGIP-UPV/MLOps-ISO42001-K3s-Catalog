...
> Prometheus in agent mode: scrapes edge workloads (including the model server metrics) and forwards them to the platform via remote-write.
...
- **Category**: `Monitoring`
- **Namespace**: `edge`
- **Reference architecture components**: `CMP-08` Infrastructure Technical Monitoring, `CMP-10` Model Technical Performance Monitoring
- **ISO/IEC 42001 Annex B clauses covered**: `B.6.1.2.2`, `B.6.2.6.1`

This chart packages **Prometheus Agent** for the **edge** tier of the reference architecture, covering the *Monitoring* capability block. It ships with defaults for K3s and a Rancher-compatible `questions.yaml`, so operators can deploy the component from the Rancher UI with guided prompts.
...
# Install this chart
helm install edge-prometheus-agent cigip-upv/edge-prometheus-agent \
  --namespace edge \
  --create-namespace
```
...
helm install edge-prometheus-agent . -n edge --create-namespace
```
...
```bash
kubectl get pods,svc,deploy,sts -A -l mlops-iso42001.cigip-upv.es/B.6.1.2.2
```
