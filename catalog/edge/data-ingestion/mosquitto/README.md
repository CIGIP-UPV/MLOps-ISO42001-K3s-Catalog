# Eclipse Mosquitto: `edge-mosquitto`

> Lightweight MQTT broker for plant-floor sensor telemetry with authentication and WebSocket support.
...
- **Namespace**: `edge`
- **Reference architecture components**: `CMP-01` Input Data Monitoring
- **ISO/IEC 42001 Annex B clauses covered**: `B.6.1.3.1`, `B.6.2.6.4`

This chart packages **Eclipse Mosquitto** for the **edge** tier of the reference architecture, covering the *Data Ingestion* capability block. It ships with defaults for K3s and a Rancher-compatible `questions.yaml`, so operators can deploy the component from the Rancher UI with guided prompts.
...
# Install this chart
helm install edge-mosquitto cigip-upv/edge-mosquitto \
  --namespace edge \
  --create-namespace
```
...
cd MLOps-ISO42001-K3s-Catalog/catalog/edge/data-ingestion/mosquitto/manifests
helm dependency build .
helm install edge-mosquitto . -n edge --create-namespace
```
...
```bash
kubectl get pods,svc,deploy,sts -A -l mlops-iso42001.cigip-upv.es/B.6.1.3.1
```
