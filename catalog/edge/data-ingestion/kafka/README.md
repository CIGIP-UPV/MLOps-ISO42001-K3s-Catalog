...
This chart packages **Apache Kafka (Edge)** for the **edge** tier of the reference architecture, covering the *Data Ingestion* capability block. It ships with defaults for K3s and a Rancher-compatible `questions.yaml`, so operators can deploy the component from the Rancher UI with guided prompts.
...
# Install this chart
helm install edge-kafka cigip-upv/edge-kafka \
  --namespace edge \
  --create-namespace
```
...
helm install edge-kafka . -n edge --create-namespace
```
...
```bash
kubectl get pods,svc,deploy,sts -A -l mlops-iso42001.cigip-upv.es/B.6.2.6.4
```
