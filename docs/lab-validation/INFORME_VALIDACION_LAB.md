...
**Origen de los datos.** Todo dato de las secciones 1 a 6 procede de ejecuciones en el laboratorio con `docs/lab-validation/run-lab-validation.sh`, cuyas evidencias están en `docs/lab-validation/raw/<ejecución>/` (una línea por prueba en `results.jsonl`, con el comando). Lo que procede del clúster local de desarrollo (Docker Desktop, kind, arm64) se indica expresamente y está en `docs/lab-validation/local-evidence/`. Las ejecuciones se listan en el anexo. El [addendum de CMP-12](./ADDENDUM_CMP12.md) valida después la interfaz de retroalimentación y documenta una incidencia que matiza este informe: con NetworkPolicy aplicadas en el nodo de plataforma, la consolidación programada fallaba hasta su corrección. Antes de publicar, las direcciones IP públicas de los nodos se sustituyeron en las evidencias por `<ip-kb2>`, `<ip-worker1-kb2>` y `<ip-edgenode01>`; es el único cambio hecho a las salidas originales.
